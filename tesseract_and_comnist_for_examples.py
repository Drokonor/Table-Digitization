...
files = os.listdir(dir + '\\Example_Tables_1_2')
cells_of_tables = []
count = 0
start_time = datetime.now()
for i in range(len(files)):
    if 'jpg' or 'png' in files[i]:
        cells_of_tables.append(image_to_cells(dir + '\\Example_Tables_1_2\\' + files[i]))
for k in range(len(cells_of_tables)):
    ...
    for i in range(len(cells)):
        ...
        for j in range(len(cells[i])):
            if j == 0 or j == 1 or i == 0:
                try:
                    img = cells[i][j].copy()
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    config = r'--oem 3 --psm 6 -c tessedit_char_blacklist=_|[]'
                    table_df[i].append(pytesseract.image_to_string(img, config=config, lang='rus'))
                except:
                    table_df[i].append('')
            else:
    ...
    df.to_excel(dir + '\\Tesseract_and_Comnist_Example_Tables\\' + files[k][:-3] + 'xls')
    count += 1
    print(count)
    print(datetime.now() - start_time)

files = os.listdir(dir + '\\Example_Tables_1-1_3')
cells_of_tables = []
for i in range(len(files)):
    if 'jpg' or 'png' in files[i]:
        cells_of_tables.append(image_to_cells(dir + '\\Example_Tables_1-1_3\\' + files[i]))
for k in range(len(cells_of_tables)):
    table_df = []
    cells = cells_of_tables[k]
    for i in range(len(cells)):
        table_df.append([])
        for j in range(len(cells[i])):
            if j == 0 or j == 1 or j == 2 or i == 0 or i == len(cells) - 1:
                try:
                    img = cells[i][j].copy()
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    config = r'--oem 3 --psm 6 -c tessedit_char_blacklist=_|[]'
